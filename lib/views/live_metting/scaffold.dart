...
import '../../../global_widgets/video_sdk/common/app_bar/meeting_appbar.dart';
import '../../../global_widgets/video_sdk/common/chat/chat_view.dart';
import '../../../global_widgets/video_sdk/common/joining/waiting_to_join.dart';
import '../../../global_widgets/video_sdk/common/meeting_controls/conference.dart';
import '../../../global_widgets/video_sdk/common/participant/participant_list.dart';
import '../../../global_widgets/video_sdk/conference-call/conference_participant_grid.dart';
import '../../../global_widgets/video_sdk/conference-call/conference_screenshare_view.dart';
...
class _ConfereneceMeetingScreenState extends State<ConfereneceMeetingScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: _joined
                  ...
                  body: Column(
                    ...
                    children: [
                      ...
                      AnimatedCrossFade(
                        ...
                        firstChild: MeetingActionConference(
                          ...
                          onMoreOptionSelected: (option) {
                            ...
                            if (option == "screenshare") {
                              if (remoteParticipantShareStream == null) {
                                ...
                                showSnackBarMessage(
                                    message:
                                        "Someone is already presenting".tr(),
                                    context: context);
                              }
                            } else if (option == "recording") {
                              if (recordingState == "RECORDING_STOPPING") {
                                showSnackBarMessage(
                                    message: "Recording is stopping".tr(),
                                    context: context);
                                ...
                                meeting.stopRecording();
                                  ...
                                  "RECORDING_STARTING") {
                                showSnackBarMessage(
                                    message: "Recording is starting".tr(),
                                    context: context);
                              } else {
          ...
          : WaitingToJoin(calling: 'Connecting...'.tr()),
    );
  }
  ...
  void registerMeetingEvents(Room _meeting) {
    ...
    _meeting.on(Events.roomLeft, (String? errorMsg) {
      if (errorMsg != null) {
        showSnackBarMessage(
            message: "${'Meeting left due to'.tr()} $errorMsg !!",
            context: context);
      }
      if (widget.isHost) {
        debugPrint('widget.isHost');
      } else {
        Get.back();
      }
    });
    ...
    _meeting.on(Events.recordingStateChanged, (String status) {
      showSnackBarMessage(
          message:
              "Meeting recording ${status == "RECORDING_STARTING" ? "is starting".tr() : status == "RECORDING_STARTED" ? "started".tr() : status == "RECORDING_STOPPING" ? "is stopping".tr() : "stopped".tr()}",
          context: context);
