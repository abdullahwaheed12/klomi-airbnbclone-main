...
class _CallStatsBottomSheetState extends State<CallStatsBottomSheet> {
                ...
                children: [
                  Flexible(
                    child: Column(
                      children: [
                        Text(
                          " ${widget.participant.displayName} ${'- Quality Metrics :'.tr()} ${score == null ? '-' : score! > 7 ? 'Good'.tr() : score! > 4 ? 'Average'.tr() : 'Poor'.tr()}",
  ...
  void updateStats() {
    ...
    double? _score = (stats.length) > 0 ? 100.0 : null;
    if (_score != null) {
    ...
    setState(() {
      score = _score != null ? _score ~/ 10 : null;
